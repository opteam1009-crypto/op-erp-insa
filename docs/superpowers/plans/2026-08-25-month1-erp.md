...
**Tech Stack:** Next.js 16 (App Router) + TypeScript, Tailwind CSS v4, Supabase (`@supabase/supabase-js`, `@supabase/ssr`), `xlsx` (SheetJS) for Excel, `zod` for validation, `date-fns` for date math, Vitest for unit tests, Vercel Cron for scheduled Slack notifications.

> Amended 2026-08-25 after Task 1 review: `create-next-app@latest` scaffolded Next.js 16 rather than the originally-planned 15. User confirmed staying on 16 (current stable) rather than downgrading — later tasks' App Router / Supabase SSR code is written against APIs stable across both majors, so no other task text changes.
...
> Amended 2026-08-25 after Task 3 review: Next.js 16.3.2 deprecates the `middleware.ts` file convention in favor of `proxy.ts` (exported function renamed from `middleware` to `proxy`; `config.matcher` unchanged). Implemented via `npx @next/codemod@canary middleware-to-proxy .`. The steps below still describe `middleware.ts` as originally planned/built — the actual shipped file is `proxy.ts`.
...
- Create: `middleware.ts` (superseded by `proxy.ts`, see amendment above)
